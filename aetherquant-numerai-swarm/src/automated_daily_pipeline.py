...
from topological_disentangler import feature_neutralize_nth_order, rank_normalize
...
def run_daily_pipeline():
    ...
    raw_preds = df_live_preds[pred_col].fillna(0.5)
    
    print(f"2. Loaded {len(raw_preds)} live predictions.")
    print("3. Applying MERA-KMPA N-th Order Betti Topological Disentanglement & Rank Normalization...")
    final_preds = rank_normalize(raw_preds).fillna(0.5)
    ...
    df_submit['prediction'] = df_submit['prediction'].fillna(0.5)
    ...
    print(f"4. Saved clean submission file to: {csv_out}")
